...
State of search procedure contains  $(\Pi, \varphi, \bar{\Pi}, \varphi^V, \varphi^U, \varepsilon, \tau)$ where 
\begin{itemize}
\item $\Pi$ is the interval constraint.
\item $\varphi$ represents the polynomial constraint.
\item $\bar{\Pi} = \bigwedge\limits_{v_i \in V} v_i \in i_i$ with $i_i \in \mathbb{I}$ is the result of DPLL procedure on $\Pi$.
...
\begin{table*}[t]
  ...
  \large 
  $\frac{\Pi \models_{SAT} \bar{\Pi} \quad \Pi^{c'} = flattern(\bar{\Pi})}{(\Pi, \varphi, \emptyset, \emptyset, \emptyset, \varepsilon, \tau) \to (\Pi, \varphi, \Pi^{c'}, \emptyset, \emptyset, \varepsilon, \tau)}$ \tiny $\Pi$\_SAT\\\\
  \large
  $\frac{\bar{\Pi} \not= \emptyset \quad   \varphi^V \wedge \varphi^U = \varphi \quad \varphi^V \cup \varphi^U = \emptyset \quad \varphi^V \text{ is }  \bar{\Pi}_{IA}\text{-VALID}}{(\Pi, \varphi, \bar{\Pi}, \emptyset, \emptyset, \varepsilon, \tau) \to (\Pi, \varphi, \bar{\Pi}, \varphi^V, \varphi^U, \varepsilon, \tau)}$ \tiny IA\_SAT \\\\  
  \large 
  $\frac{\varphi^V = \varphi}{(\Pi, \varphi, \bar{\Pi}, \varphi^V, \varphi^U, \varepsilon, \tau) \to SAT}$ \tiny IA\_VALID \\\\
  \large 
  $\frac{\bar{\Pi} \not= \emptyset \quad \varphi^U \not= \emptyset \quad \varphi^U \text{ is }  \bar{\Pi}_T\text{-SAT}}{(\Pi, \varphi, \bar{\Pi}, \varphi^V, \varphi^U, \varepsilon, \tau) \to SAT}$ \tiny TEST\_SAT \\\\
  \large     
  $\frac{\varphi^U \text{ is }  \bar{\Pi}_T\text{-UNSAT} \quad \bar{\Pi} = \bigwedge\limits_{v_i \in V} v_i \in \langle l_i, h_i \rangle \quad \forall i (h_i - l_i < \varepsilon)}{(\Pi, \varphi, \bar{\Pi}, \varphi^V, \varphi^U, \varepsilon, \tau) \to (\Pi \wedge \neg \bar{\Pi}, \varphi, \emptyset, \emptyset, \emptyset, \varepsilon, \top)}$ \tiny THRESHOLD \\\\
  \large 
  $\frac{\varphi^U \text{ is }  \bar{\Pi}_T\text{-UNSAT} \quad \bar{\Pi} = \bigwedge\limits_{v_i \in V} v_i \in \langle l_i, h_i \rangle \quad \exists j(h_j - l_j > \varepsilon) \quad l_j < d \in \mathbb{R} < h_j \quad I_j = v_j \in \langle l_j, h_j \rangle \quad I_{j1} = x_j \in \langle l_j, d\rangle \quad I_{j2} = x_j \in \langle d, h_j \rangle}{(\Pi, \varphi, \bar{\Pi}, \varphi^V, \varphi^U, \varepsilon, \tau) \to (\Pi \wedge (\neg I_j \vee I_{j1} \vee I_{j2}) \wedge (I_j \vee \neg I_{j1}) \wedge (I_j \vee \neg I_{j2}) \wedge (\neg I_{j1} \vee \neg I_{j2}), \varphi, \emptyset, \emptyset, \emptyset, \varepsilon, \tau)}$ \tiny REFINE \\\\
  \large 
  $\frac{\{\varphi_i| \varphi_i \wedge \varphi'_i = \varphi; \; \varphi_i \text{ is } \bar{\Pi}_{IA}\text{-UNSAT}\} \quad var(\varphi_i) = \{x_{ij}|j=1,\cdots, m_i\} \quad (I_{ij} = x_{ij} \in [l_{ij}, h_{ij}]) \in \bar{\Pi}}{(\Pi, \varphi, \bar{\Pi}, \emptyset, \emptyset, \varepsilon, \tau) \to (\Pi \wedge \bigwedge\limits_i \bigvee\limits_{j=1}^{m_i} \neg I_{ij}, \varphi, \emptyset, \emptyset, \emptyset, \varepsilon, \tau)}$ \tiny IA\_UNSAT \\\\
  \hline\\
  \end{tabular}
...
\begin{theorem}
Let $(\Pi, \varphi, \bar{\Pi}, \varphi^V, \varphi^U, \varepsilon, \tau)$ be any state of our system, then the following properties are invariants:
\begin{enumerate}
\item $\bar{\Pi}_\mathbb{R} \subseteq T^p_\mathbb{R}$
\item $\varphi^V$ is $\bar{\Pi}_{R}$-VALID.
...
\begin{itemize}
\item In the case of IA\_VALID, we have $\varphi^V$ is $\bar{\Pi}_{R}$-VALID (invariant 2) $\implies \varphi^V$ is $\bar{\Pi}_\mathbb{R}$-SAT $\implies \varphi$ is $\bar{\Pi}_\mathbb{R}$-SAT (because $\varphi^V = \varphi$ is the condition of this transition). In addition, following invariant 1, we have $\bar{\Pi}_\mathbb{R} \subseteq T^p_\mathbb{R}$, then $\varphi$ is $T^p_\mathbb{R}$-SAT (Lemma ~\ref{lemma:subtheory-SAT}).
\item \sloppy In the case of TEST\_SAT, $\varphi^U$ is $\bar{\Pi}_\mathbb{R}$-SAT $\implies \exists M \in \bar{\Pi}_\mathbb{R} \; \models_M \varphi^V$. Let ${M^c_\mathbb{R} \in \bar{\Pi}_\mathbb{R}}$ such that $\models_{M^c_\mathbb{R}} \varphi^V$ which implies $(\varphi^V)^{M^c_\mathbb{R}} = 1$. In addition, because $\varphi^V$ is $\bar{\Pi}_\mathbb{R}$-VALID (invariant 2) and $M^c_\mathbb{R} \in \bar{\Pi}_\mathbb{R}$, we have $\models_{M^c_\mathbb{R}} \varphi^U$ or ${(\varphi^U)^{M^c_\mathbb{R}} = 1}$. Consider the evaluation of $\varphi$ under the model $M^c_\mathbb{R}$: ${(\varphi)^{M^c_\mathbb{R}} = (\varphi^U \wedge \varphi^V)^{M^c_\mathbb{R}}}$ (invariant 3) $= min((\varphi^U)^{M^c_\mathbb{R}}, (\varphi^V)^{M^c_\mathbb{R}}) = min(1, 1) = 1 \implies \models_{M^c_\mathbb{R}} \varphi \implies \varphi$ is $\bar{\Pi}_\mathbb{R}$-SAT $\implies \varphi$ is $T^p_\mathbb{R}$-SAT (because of invariant 1 and Lemma ~\ref{lemma:IA-R-OP})
\end{itemize}
