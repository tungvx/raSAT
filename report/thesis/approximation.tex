...
\item $\forall p \in P^p$; $I^p_{IA}(p)=\{(i_1, i_2)| i_1, i_2 \in M^p_{IA} \text{ and } i_1 \; p_{IA} \; i2\}$. The definition of $ p_{IA}$ is as follow:
\begin{itemize}
\item $[l_1, h_1] \succ_{IA} [l_2,  h_2] \iff l_1 > h_2$
\item $[l_1, h_1] \prec_{IA} [l_2,  h_2] \iff h_1 < l_2$
\item $[l_1, h_1] \succeq_{IA} [l_2,  h_2] \iff l_1 \ge h_2$
\item $[l_1, h_1] \preceq_{IA} [l_2,  h_2] \iff h_1 \le l_2$
\item $[l_1, h_1] \approx_{IA} [l_2,  h_2] \iff l_1 = h_1 = l_2 = h_2$
\item $i_1 \not\approx_{IA} i_2 \iff i_1 \succ_{IA} i_2 \vee i_1 \prec_{IA} i_2$
\end{itemize}
\item $\forall f \in F^p \setminus \{\mathbf{0}, \mathbf{1}\}$; $I^p_{IA}(f) = U^p_{IA} \times U^p_{IA} \mapsto U^p_{IA}$ such that $ I^p_{IA}(f)(i_1, i_2)= i_1 \; f_{IA} \; i_2$ where $f_{IA}$ satisfies the following properties:
\begin{itemize}
...
\end{itemize}
\item $I^p_{IA}(\mathbf{0}) = [0,0]$ and $I^p_{IA}(\mathbf{0}) = [1,1]$
...
Suppose $\varphi^p$ is not $\Pi^p_\mathbb{R}$-UNSAT, that means it is either  $\Pi^p_\mathbb{R}$-SAT or  $\Pi^p_\mathbb{R}$-VALID. In either case, there exist at least a model $M^p_\mathbb{R} \in \Pi^p_\mathbb{R}$. 
\end{proof}
