...
\begin{definition}
Let $\Sigma$ is a signature. A $\Sigma$-theory T is a (infinite) set of $\Sigma$-model.
\end{definition}

\begin{definition}
Let $\Sigma = (S, P, F, \alpha)$, $\varphi$ and $M=(U, I)$ are a signature, a $\Sigma-$formula and a $\Sigma-$model respectively. The evaluation of $\varphi$ against $M$ which is denoted by $\varphi^M$ is defined recursively as:
\begin{enumerate}
...
Given any $\Sigma$-model $M$ and $\Sigma$-formula $\varphi$, we have $\models_M \varphi \iff \models_M \neg \varphi$
\end{lemma}

\begin{proof}
$\models_M \varphi \iff \varphi^M = true \iff \neg \varphi^M = false \iff (\neg \varphi)^M = false \iff \models^M \neg \varphi$
\end{proof}

\begin{definition}
Let $T = \{M\_i | i = 1, 2, \cdots \}$ be a $\Sigma$-theory. A $\Sigma$-formula $\varphi$ is:
\begin{itemize}
\item satisfiable in $T$ or T-SAT iff $\exists i\models_{M_i} \varphi$
\item valid in $T$ or T-VALID iff $\forall i \models_{M_i} \varphi$
\item unsatisfiable in $T$ or T-UNSAT iff $\forall i \not\models_{M_i} \varphi$
\end{itemize}
\end{definition}

\begin{lemma} \label{lemma:theory-valid-unsat}
If $T = \{M_i | i = 1, 2, \cdots\}$ be a $\Sigma$-theory, then $\varphi$ is T-VALID $\iff$ $\neg\varphi$ is T-UNSAT
\end{lemma}

\begin{proof}
$\varphi$ is T-VALID $\iff \forall i \models_{M_i} \iff \forall i \not\models_{M_i} \neg\varphi$ (Lemma \ref{lemma:model-sat-unsat}) $\iff \neg\varphi$ is T-UNSAT.
\end{proof}
...
State of search procedure contains  $(BC, PC, CB, PC^V, PC^U)$
...
\begin{table*}[t]
  ...
  \hline\\
  \large 
  $\frac{BC \models_{SAT} \bot}{(BC, PC, \emptyset, \emptyset, \emptyset) \to UNSAT}$ \\\\
  \large 
  $\frac{BC \models_{SAT} CB}{(BC, PC, \emptyset, \emptyset, \emptyset) \to (BC, PC, CB, \emptyset, \emptyset)}$ \\\\
  $\frac{CB \not= \emptyset \qquad PC^V \subseteq PC \qquad PC^U \subseteq PC \qquad PC^V \cap PC^U = PC \qquad PC^V \cup PC^U = \emptyset \qquad PC^V \text{ is }  T^O\text{-VALID} \qquad PC^U \text{ is }  T^O\text{-SAT}}{(BC, PC, CB, \emptyset, \emptyset) \to (BC, PC, CB, PC^V, PC^U)}$ \\\\  
  \large 
  $\frac{PC^U \text{ is }  T^U\text{-SAT}}{(BC, PC, CB, PC^V, PC^U) \to SAT}$ \\\\
  \large 
  $\frac{PC^U \text{ is }  T^U\text{-UNSAT} \qquad (I_j = x_j \in [l_j, h_j]) \in CB \qquad d > l_j \qquad d < h_j \qquad I_{j1} = x_j \in [l_j, d] \qquad I_{j2} = x_j \in [d, h_j]}{(BC, PC, CB, PC^V, PC^U) \to (BC \wedge (\neg I_j \vee I_{j1} \vee I_{j2}) \wedge (I_j \vee \neg I_{j1}) \wedge (I_j \vee \neg I_{j2}) \wedge (\neg I_{j1} \vee \neg I_{j2}), PC, \emptyset, \emptyset, \emptyset)}$ \\\\
  \large 
  $\frac{\{pc_i| i = 1,\cdots, n\} \subseteq PC^U \qquad pc_i \text{ is }  T^O\text{-UNSAT} \qquad var(pc_i) = \{x_{ij}|j=1,\cdots, m_i\} \qquad (I_{ij} = x_{ij} \in [l_{ij}, h_{ij}]) \in CB}{(BC, PC, CB, \emptyset, \emptyset) \to (BC \wedge \bigwedge\limits_{i=1}^n \bigvee\limits_{j=1}^m \neg I_{ij}, PC, \emptyset, \emptyset, \emptyset)}$ \\\\
  \hline\\
  \end{tabular}
