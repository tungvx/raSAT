...
\begin{lemma}
$\forall i_1, i_2 \in U^p_{IA}; \forall r_1 \in i_2, r_2 \in i_2; \forall p \in P^p (i_1 \; p_{IA} \; i_2 = true \implies r_1 \; p_\mathbb{R} \; r_2 = true)$
\end{lemma}
...
\end{itemize}
Suppose that $i_1 \; p_{IA} \; i_2$, we need to prove that $r_1 \; p_\mathbb{R} \; r_2$ by considering all the possible cases of $p$:
\begin{enumerate}
\item If $p$ is $\succ$, the we have $[l_1, h_1] \succ_{IA} [l_2, h_2] \implies l_1 > h_2 \implies r_1 > r_2$ (because $r_1 \ge l_1$ and $r_2 \le h_2$) $\implies r_1 \succ_\mathbb{R} r_2$.
\item If $p$ is $\prec$, the we have $[l_1, h_1] \prec_{IA} [l_2, h_2] \implies h_1 < l_2 \implies r_1 < r_2$ (because $r_1 \le h_1$ and $r_2 \ge l_2$) $\implies r_1 \prec_\mathbb{R} r_2$.
\item If $p$ is $\succeq$, the we have $i_1 \succeq_{IA} i_2 \implies \neg (i_1 \prec i_2) \implies \neg() r_1 < r_2$ (because $r_1 \le h_1$ and $r_2 \ge l_2$) $\implies r_1 \prec_\mathbb{R} r_2$.
\end{enumerate}
\end{proof}
...
\begin{proof}
In order to prove Theorem ~\ref{lemma:IA-OT}, we need to prove that $\forall \varphi^p (\varphi^p$ is $\{\Pi^p_{IA}\}$-UNSAT $\implies \varphi^p$ is $\Pi^p_\mathbb{R}$-UNSAT). 
