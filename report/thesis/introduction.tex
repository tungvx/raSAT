...
\begin{enumerate}
\item
Satisfiable inequalities allow over-approximation. An over-approximation estimates the range of a polynomial $f$ as $range^O(f)$ that covers all the possible values of $f$, i.e. $range(f) \subseteq range^O(f)$. For an inequality $f>0$, if $range^O(f)$ stays in the positive side, it can be concluded as SAT. On the other hand, over-approximation cannot prove the satisfiability of SAT equations.
\item
...
\begin{enumerate}
\item If $range^O(f)$ resides on the negative side, $f > 0$ is UNSAT.
\item If $range^O(f)$ stays on either negative or positive side, $f = 0$ is UNSAT.
\end{enumerate}
