...
Our aim is an SMT solver for solving polynomial constraint. We first focus on strict inequalities because of they allow approximations.
\begin{enumerate}
\item
Inequalities allow under-approximation. Under-approximation computes a range $U.range(f)$ of a polynomial $f$ such that $range(f) \supset U.range(f)$. On the other hand, over-approximation estimates a range $O.range(f)$ of a polynomial $f$ that covers all the possible values of $f$, i.e. $range(f) \subset O.range(f)$. For an inequality $f>0$, if $O.range(f)$ stays in the negative side
\item
For the SAT case, if $f(\bar{x}) > 0$ has a real solution $\bar{x_0}$, there exist rational points near $\bar{x_0}$ which are also solutions of the inequality. 
\end{enumerate}
\begin{comment}
