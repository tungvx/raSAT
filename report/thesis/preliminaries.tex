...
\begin{itemize}
\item satisfiable in $T$ or T-SAT if and only if for all $M \in T$ we have $\models_{M} \varphi$,
\item valid in $T$ or T-VALID if and only if for all $M \in T$ we have $\models_{M} \varphi$,
\item unsatisfiable in $T$ or T-UNSAT if and only if for all $M \in T$ we have $\not\models_{M} \varphi$, and
\item unknown in $T$ or T-UNKNOWN if and only if for all $M \in T$, $\varphi^M = \mathring{u}$.
\end{itemize}
\end{definition}
...
\begin{itemize}
\item If we consider each formula $p(f_1, f_2)$ as an propositional symbols, we can first convert $\varphi$ into an CNF formula and then use DPLL procedure to infer a sequence of literals that satisfies $\varphi$ (in terms of propositional logic).  
\item The sequence of literals may contains some literals of the form $\neg p(f_1, f_2)$. However, from the semantics of polynomial constraints, we can change:
\begin{center}
\begin{tabular}{r c l}
