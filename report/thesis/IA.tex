...
A model $M^p_{AI} = (U^p_{AI}, I^p_{AI})$ over intervals contains a set of all intervals $U^p_{AI} = U^p_{IA}$ and a map $I^p_{CI}$ that satisfies the following conditions.
\begin{enumerate}
\item $I^p_{AI}(Real) = I^p_{IA}(Real)$
\item $\forall p \in P^p$; $I^p_{AI}(p)= I^p_{IA}(p)$
