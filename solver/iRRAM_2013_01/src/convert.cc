...
INTEGER::INTEGER(const DYADIC& y)
{
	MP_int_init(value);
...
//Conversions from REAL to smaller types
DYADIC::DYADIC(const REAL &x)
{ MP_init(value);
  (*this)=approx(x,DYADIC::getprec());
}

//Conversion from REAL to double with a relative precision of p bits
double REAL::as_double(int p) const {
