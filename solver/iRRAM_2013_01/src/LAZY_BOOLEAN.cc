...
LAZY_BOOLEAN::operator bool() const {
  bool result;
  if ( unlikely((ACTUAL_STACK.inlimit==0)) && iRRAM_thread_data_address->cache_b.get(result)) return result;

  if ( unlikely(value == BOTTOM )){
    DEBUG1(1,"lazy boolean value BOTTOM leading to iteration\n");
    REITERATE(0);
  }

  result=value;
  if ( unlikely(ACTUAL_STACK.inlimit==0) ) iRRAM_thread_data_address->cache_b.put(result);
  return result;
};
...
LAZY_BOOLEAN operator && 
   (const LAZY_BOOLEAN& x, const LAZY_BOOLEAN& y)
   {
     ...
     if (x.value==BOTTOM || y.value==BOTTOM ) return BOTTOM;
