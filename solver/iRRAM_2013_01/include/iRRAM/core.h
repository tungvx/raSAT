...
// sizetype_add3(x,y,prec) **************************************************
// Increment x by y and 1*2**prec
// The resulting value may be a bit larger than the exact value,
// The resulting value may never be smaller than the exact value
// ********************************************************************

inline void sizetype_add3(sizetype& x,const sizetype& y,const sizetype& z,const  SIZETYPEEXPONENT prec) 
{ SIZETYPEEXPONENT exponent;
  x.exponent=max3(y.exponent,z.exponent,prec);
  x.mantissa=scale(y.mantissa,(exponent-y.exponent))
            +scale(z.mantissa,(exponent-z.exponent)) 
            +scale(1,(exponent-prec)) +2 ;
  x.exponent=exponent;
  sizetype_normalize(x);
}
