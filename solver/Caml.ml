...
module Caml = struct
  ...
  let var_exp e litID = 
    let lstVars = red_list (bool_vars e) in
...
let rec bool_toString = function
  ...
  (*evaluate the bound of poly expression by type of interval arithmetic*)						     let poly_eval e ia assIntv = 
  ...
  *)
  ...
  let length_cons e = List.length (red_list (bool_vars e))
  ...
  let rec rel_all l1 l2 lstVar = match l1 with
	...
	let l = red_list (e_listVar tmp) in
  ...
  let group_cons listAss listVar = 
    ...
    let new_varlist = sub_list listVar (red_list (e_listVar l1)) in
  ...
  let rec sat_find1 (e1, d1) r e vass ass = match ass with
    |[] -> (e1, d1, r)
    |h::t -> 
        let assign = List.combine vass h in
	let (res, d) = checkValue1 e assign in
	let (e2, d2) = if d < d1 then (h, d)
	               else (e1, d1) in
	if (res = 1) then
  ...
  (*find test cases that satisfying a constraint *)
  let find_tc lvars oAss e uAss = 
    let vars = red_list (bool_vars e) in
    let tc = get_index_all [] vars uAss in 
    let big_number = max_float in 
    if (tc != []) then (
      let (l1, l2) = extract [] [] tc in
  ...
  let search_tc lstAss assIntv =
      ...
      let vars_list = red_list (e_listVar lstAss) in
  ...
  *)
  ...
  (* find the clause which has many variables join with lstVar*)
    let rec find_join_clause  e lstVar lstConstr = match lstConstr with
      |[] -> e	  
      |h::t -> 
	  let l = red_list (bool_vars h) in
	  let le = red_list (bool_vars e) in
  ...
  let rec first_search lvars oAss uAss first_list = match first_list with
    |[] -> ([], lvars, oAss, uAss, [])
    |_ -> (
	let first_cons = List.hd (List.sort compare_cons first_list) in
  ...
  let rec search_inside lvars oAss uAss uk_cl = match uk_cl with
    |[] -> ([], lvars, oAss, [])
    |_ -> (
	(*
	let lstVar = (
  ...
  let rec sub_constraints e l = match l with
    |[] -> []
    |h::t ->
	let e_vars = red_list (bool_vars e) in
	let h_vars = red_list (bool_vars h) in
  ...
  |[] -> n
  |h::t -> 
    let l1 = red_list (bool_vars h) in
  ...
  |[] -> res
  |h::t ->
    let lvars = red_list (bool_vars h) in
    let dep = get_dep 0 lvars olst in
    if (dep > maxdep) then
  ...
  let search_tc2 uk_cl assIntv strTestUS esl =      
      (*GENERATE TEST CASE: CAN BE GENERATE MORE TEST CASES HERE*)    
      Random.self_init();
      let lstTc = genAss_test [] gen_test21 assIntv esl in
	...
	if (strTestUS = "") then (
	  ...
	  let e = List.hd new_uk_cl in
	  let remains = List.tl new_uk_cl in
	  let lst_vars = red_list (bool_vars e) in
	  let maxdep = get_dep 1 lst_vars remains in
	  get_maxdep e maxdep remains remains 
	)  
	else (	  
      ...
      let sub = sub_constraints first_cons uk_cl in
      let first_list = List.append sub [first_cons] in
  ...
  *)
  ...
  let rec ass_decomp_pn ass lstVarID code esl = match ass with
	...
	let (s3, s4) = var_decomp_pn h lstVarID code in
	if (s1 = "") then
	   (s3, s4^s2, (string_of_int (code+1)) ^ " " ^ svars )
	else 
	   ("(ic "^s3 ^" "^s1^")", s4^s2, (string_of_int (code+1)) ^ " " ^ svars )		   
  ...
  let dynamicDecom_pos assIntv dIntv lstVarID iVar uk_cl esl = 
    ...
    let pos_ = red_list pos in
    let neg_ = red_list neg in
	...
	let (sInterval, sLearn, bump_vars) = ass_decomp_pn red_pos lstVarID (iVar + 1) esl in
	if (dIntv <> "") then
 ...
 *)	     
 ...
 (*Get the sign in an multiplication expression*)
  ...
  (*Get positive and negative sub expression*)
  ...
  |[] -> true
  |h::t ->
      let lstVars = red_list (bool_vars h) in
  ...
  let dynTest sIntv dIntv sAss strCheck ia esl strTestUS =      let olstCheck = toIntList strCheck in
      let eIntv = sub_intv (read sIntv) in 
	...
	if (dIntv <> "") then (
      ...
      let (res, us, uk_cl) = eval_all 1 "" [] eAss ia assIntv checkVarID in

      if (res = -1) then (*if existing unsat clause*)
	(res, us, "", "", dIntv, "", "", tmp)
      else if (res = 1) then (*if all clauses are satisfiable*)
	(res, "", "", (allLog eAss ia assIntv), dIntv, "", "", tmp)      
      else (*if unknown, testing will be implemented here*)
      (
	 ...
	 (
	   ...
	   let (sInterval, sLearn, bump_vars, isDecomp) = dynamicDecom_pos assIntv dIntv checkVarID iVar clTest_US esl in
	   ...
	   if (isDecomp) then
  	     (-2, "", sLearn, "", sInterval, toPrefix (List.hd clTest_US), bump_vars, tmp)	  
	   else
	     (res, "", sLearn, "", sInterval, toPrefix (List.hd clTest_US), bump_vars, tmp)	  
	 )
	)
      )
...
(*Represente a polynomial expression by a string*)
let rec smt_poly_toString sign  = function
  | Real c -> sign_simp sign c
  | Var x -> sign ^ x
  | SubVar u -> sign ^ u
  | Mul (Real 1., Var x) -> sign ^ x
...
(*Represent a bool expression by a string*)
let rec smt_bool_toString = function
  | BVar bVar -> bVar
...
let rec smt_poly_toPrefix = function
  | Real c -> string_of_float c
  | Var x -> x
  | SubVar u -> u
...
let rem_var1 e =
  let lstVars = red_list ( smt_get_vars e) in
  div_func e lstVars
...
let rec refor_expr e (c: float) = match e with
  ...
  | Sub(Real a, e1) -> refor_expr e1 (c+.a)
  | _ -> (e, Real c, '.')
...
(*
  (*simplify expression*)
  let simp_expr = simplify_bool ori_expr in

  (*remove not and div in expressions*)
  let red_expr = reduce_expr simp_expr in

  (*Placed constants to a left/right of equations*)
  let final_expr = bool_simp red_expr in

  (*Get bound constraints of variables from assert expression eAss*)
  let bound_cons = remov_nil (getBound final_expr)in
  ...
  let strAss = "(assert "^(bool_toPrefix final_expr)^")" in
