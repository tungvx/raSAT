...
%{\bf raSAT} prepares various IAs. %as $O.T$ as in following, 
...
\item[Hong] has 20 inequality problems among 20, tuned for QE-CAD. % and quite artificial. 
\end{description}
...
\end{center}
\caption{Different options in incremental widening and deepening} 
\label{tab:incremental}
\end{table}


%We perform experiments only on inequations of Zankl, and Meti-Tarski families. 
Table~\ref{tab:rasat-experiments} shows the results of the strategy combinations (below)
on QD\_NRA/Zankl and Meti-Tarski. 
...
%\subsection*{Experiments with test case generation using variable sensitivity}
\noindent
...
Section~\ref{sec:eq} shows equality handling, but current implementation has no effective strategies
on finding a box on which all inequations are IA-valid and a decomposition $V_j$'s. 
