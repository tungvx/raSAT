...
\medskip \noindent
\textbf{Bit-blasting}. %Bit-blasting in bounded bit width is often used in SMTs for QF\_NIA. 
...
%applies it 
for integer and rational numbers respectively. %with symbolic representations for prefixed algebraic numbers. 
...
\medskip \noindent
\textbf{Linearization}. %Linearization of polynomials is often used over integers, such as
...
\sloppy
\begin{definition}
A (bounded) polynomial inequality constraint is
\[\varphi: \exists x_1 \in I_1 \cdots x_n \in I_n. \bigwedge \limits_{j=1}^m \psi_j(x_1,\cdots,x_n)\]
...
${\exists x_1 \ldots x_n. (\bigwedge \limits_{i=1}^n x_i \in I_i) \wedge (\bigwedge \limits_{j=1}^m \psi_j(x_1,\cdots,x_n))}$. 
We call ${\bigwedge \limits_i x_i \in I_i}$ an {\em interval constraint}, and 
we refer $\bigwedge \limits_{j=1}^m \psi_j(x_1,\cdots,x_n)$ by $\psi(x_1,\cdots,x_n)$. We denote the set of solutions of the constraint $\psi(x_1,\cdots, x_n)$ as ${\mathbb{S}(\psi(x_1,\cdots, x_n)) = \{(r_1,\cdots, r_n) \in \Real^n \mid \psi(r_1. \cdots, r_n) \text{ holds}\}}$.
Because our methodology is an extension of ICP, next section is going to review ICP before the details of \textbf{raSAT} loop.
...
Algorithm~\ref{Al:ICP} describe the standard ICP methodology for solving polynomial inequalities. Two functions $prune(B,\psi)$ and $branch(B)$ satisfies the following properties:
\begin{itemize}
\item If $B' = prune (B, \psi)$, then $B' \subseteq B $ and $ B' \cap \mathbb{S}(\psi) = B \cap \mathbb{S}(\psi)$
\item If $\{B_1, B_2\} = branch (B)$, then $B = B_1 \cup B_2$ and $B_1 \cap B_2 = \emptyset$
\end{itemize}
...
\While {$S \neq \emptyset$}
  ...
  \Else \Comment \emph{IA} cannot conclude the constraint $\implies$ \emph{Refinement} Step
  	\State $\{B_1, B_2\} \gets branch(B')$ \Comment split $B'$ into two smaller boxes $B_1$ and $B_2$	
  	\State $S \gets (S \setminus \{B\}) \cup \{B_1, B_2\}$
  \EndIf
\EndWhile
\State \Return UNSAT
\end{algorithmic}
\caption{ICP for solving the constraint $\psi = \bigwedge\limits_{j=k}^mg_j(x_1, \cdots, x_n) > 0$ given the initial box $B_0 = I_1 \times \cdots \times I_n$}
\label{Al:ICP}
\end{algorithm}
\noindent
...
Although an ICP is enough to implement an SMT solver, 
we extend it as {\bf raSAT} (SAT by refinement of approximations) loop to accelerate SAT detection 
by adding testing as an under-approximation, which works as in Algorithm~\ref{Al:raSATLoop}. %Fig.~\ref{fig:OTrefine}~(b).
\begin{comment}
\begin{enumerate}
\item When Interval Arithmetic detects UNSAT (resp. valid), 
answer UNSAT (resp. SAT). 
\item When an testing detects SAT, answer SAT. 
...
\While {$\Pi$ is satisfiable} \Comment Some more boxes exist
  \State $\pi = \{x_i \in I_{ik} \mid i \in \{1,\cdots, n\}, k \in \{1,\cdots, i_k\} \} \gets $ a solution of $\Pi$ 	
  \State $B \gets $ the box represented by $\bigwedge\limits_{i=1}^n\bigwedge\limits_{k=1}^{i_k}x_i \in I_{ik}$
  ...
  \ElsIf {$B$ satisfies $\psi$ by using \emph{Testing}} \Comment Different from ICP
    \State \Return SAT
  \Else \Comment Neither \emph{IA} nor \emph{Testing} conclude the constraint $\implies$ \emph{Refinement} Step
  	\State $(x_i \in I_{ik}) \in \pi$ such that $\forall k_1 \in \{1,\cdots, i_k\} I_{ik} \in I_{ik_1}$
  	\State $\{I_1, I_2\} \gets branch(I_{ik})$ \Comment split $I_{ik}$ into two smaller intervals $I_1$ and $I_2$
  	\State $\Pi \gets \Pi \wedge (x_i \in I_{ik} \leftrightarrow (x_i \in I_1 \vee x_i \in I_2)) \wedge \neg(x_i \in I_1 \wedge x_i \in I_2)$
  \EndIf
\EndWhile
\State \Return UNSAT
\end{algorithmic}
\caption{\textbf{raSAT} loop for solving the constraint $\psi = \bigwedge\limits_{j=k}^mg_j(x_1, \cdots, x_n) > 0$ given the initial box represented by $\Pi = \bigwedge\limits_{i=1}^n x_i \in I_i^0$}
\label{Al:raSATLoop}
\end{algorithm}
...
for selection of a variable to decompose (line 11 of Algorithm~\ref{Al:raSATLoop}) and a box to explore line 13 of Algorithm~\ref{Al:raSATLoop}. 
...
Affine Interval~\cite{Comba93affinearithmetic} introduces \emph{noise symbols} $\epsilon$, 
...
\bigwedge \limits_{j=1}^m g_j > 0$, and so on (Fig.~\ref{fig:incwid} (a)). 
...
\sloppy
Let $\varphi = \exists x_1 \in I_1 \cdots x_n \in I_n. \bigwedge \limits_{j=1}^m g_j > 0$ 
becomes ${\vee~( \exists x_1 \in I'_1 \cdots x_n \in I'_n. \bigwedge \limits_{j=1}^m g_j > 0)}$ 
...
If every $g_j > 0$ is SAT, $F$ is SAT. 
Thus, if the box $I'_1 \times \cdots \times I'_n$ needs to be explore, it must contain 
a test-UNSAT API (thus IA-SAT). 

We denote the estimated range of $g_j$ for $x_1 \in I'_1 \cdots x_n \in I'_n$ with IA
by $range(g_j, I'_1 \times \cdots \times I'_n)$. 
If IA is an Affine Interval, 
it is in the form ${(c_1,d_1)\epsilon_1 + \cdots + (c_n,d_n)\epsilon_n}$, 
and we can obtain $range(g_j, I'_1 \times \cdots \times I'_n)$ by instantiating $\epsilon_i$ with $(-1,1)$ for $i \in \{1, \cdots, n\}$. 
We define 
\begin{itemize} 

\item {\em Sensitivity} of a variable $x_i$ in an API $g_j > 0$ is $max(|c_i|, |d_i|)$. 
\item {\em SAT-likelihood} of an API $g_j > 0$ is $| I \cap (0,\infty) | / |I|$ where $I = range(g_j, I'_1 \times \cdots \times I'_n)$, and 
...
\begin{itemize}
\item sensitivity is estimated as $1$ for $x$ and $2$ for $y$ by $AF_2$, and $3\frac{1}{4}$ for $x$ and 
$2$ for $y$. 
...
based on MiniSat 2.2 as a backend SAT solver and the library in \cite{Al2012.14} for round-down/up in Interval Arithmetics. 
...
We run some options for incremental widening and incremental deepening on inequalities of Zankl family in order to select the best combination. From Table~\ref{tab:incremental} we prefer the following setting:
\begin{itemize}
...
We compare {\bf raSAT} with other SMT solvers on NRA benchmarks: Zankl, Meti-Tarski and Keymaera. 
The timeout is $500s$. 
...
while that of {\bf raSAT}, \textbf{dReal-2.15.01} and {\bf Z3 4.3} means  UNSAT over $[-\infty, \infty]$. Another note is that if \textbf{dReal-2.15.01} concludes SAT, the constraint is $\delta$-SAT, which cannot imply the satisfiability of the constraint. %For instances, with a number of UNSAT problems in Zankl, \textbf{dReal} still concludes SAT.

Table~\ref{tab:comparison} shows the results of experiments.
...
For $\varphi = \exists x_1 \in I_1 \cdots x_n \in I_n (\bigwedge \limits_{j}^m f_j > 0~\wedge~g = 0)$. Suppose decomposition creates a box $I = (l_1, h_1) \times \cdots \times (l_n, h_n)$ where $(l_i, h_i) \subseteq I_i$ for all $i \in \{1, \cdots, n\}$, such that $\bigwedge \limits_{j}^m f_j > 0$ is IA-VALID in the box. Let $(l_g, h_g) = range(g, I)$.
\begin{enumerate}[(i)]
...
{\bf raSAT} loop is easily modified to QF\_NIA (nonlinear arithmetic over integers) from QF\_NRA, 
by setting $\gamma_0 = 1$ in incremental deepening in Section~\ref{sec:incsearch} 
and restricting testdata generation on intergers. 
We also compare {\bf raSAT} with {\bf Z3 4.3} on benchmarks of QF\_NIA/AProVE. 
{\bf AProVE} consists of 6850 inequalities and 1979 equalities. 
Some has several hundred variables, but each API has few variables (mostly just 2 variables). Note that the use of the Intermediate Value Theorem cannot be applied for QF\_NIA constraints since the polynomials are not continuous. However, Interval Arithmetics can conclude UNSAT of QF\_NIA benchmarks because UNSAT over real numbers simply implies UNSAT over integer numbers. For SAT benchmarks (both inequalities and equalities), \textbf{raSAT} concludes satisfiability only by testing.
