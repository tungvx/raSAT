...
the form $p_j(x_1,...,x_n) > 0$ and $I_1, \cdots, I_n \in \mathbb{I}$. % with $p_j(x_1,...,x_n)$ is a polynomial.
...
The \textbf{raSAT} loop is implemented in an SMT solver \textbf{raSAT} with the following extension to handle polynomial constraints over the integers:
...
\begin{itemize}
\item setting $\gamma_0 = 1$ in
the incremental deepening, and
\item restricting test data generation on integer numbers. 
\end{itemize}
