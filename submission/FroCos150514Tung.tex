...
Let $\psi = \bigwedge\limits_{j=1}^mg_j(x_1, \cdots, x_n) > 0$ be a target constraint. Algorithm~\ref{Al:ICP} describe the basic ICP for solving polynomial inequalities
\begin{algorithm}
\begin{algorithmic}[1]
...
\item[dReal] It has different judgments on SAT, called $\delta$-SAT, which allows the deviation of the width $\delta$. 
